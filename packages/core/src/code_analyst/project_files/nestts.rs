...
pub fn get_project_files(project_dir: LocalPath) -> Vec<String> {
    ...
    let ts_files = searcher.find_files_with_extensions(vec!["ts"]);
    match maints.get(0) {
    ...
    for ts_file in ts_files {
        add_file_and_imports(&mut files, &project_dir.as_path(), &ts_file);
    }
    // remove duplicates
    files.sort();
    files.dedup();
    // remove any files that have ".." in their path
