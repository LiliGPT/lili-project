...
import { CodeProjectEntity as CodeProject } from '../types/code-project.types';
...
export interface ProjectState extends EntityState<CodeProject> {
  loadingStatus: ReduxLoadingStatus,
  error?: string | null;
}

export const projectAdapter = createEntityAdapter<CodeProject>();
 ...
 * ```
 */
export const fetchProject = createAsyncThunk<CodeProject[]>(
  'project/fetchStatus',
  async (_, thunkAPI) => {
    /**
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state: ProjectState, action: PayloadAction<CodeProject[]>) => {
